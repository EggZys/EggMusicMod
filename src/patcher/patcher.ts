...
export async function processBuild(build: AppBuild) {
  ...
  packageJsonContents.name = "EggMusicMod";
  packageJsonContents.author = "EggZys [github.com/EggZys]";
  ...
  await $`bun install --ignore-scripts`.cwd(buildModdedDir);

  logProgress(`✔️   Done (standalone installer build skipped - using asar replacement)`);
